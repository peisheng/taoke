...
namespace TAOKE.Tests
{
    public abstract class TAOKETestBase : AbpIntegratedTestBase<TAOKETestModule>
    {
        private DbConnection _hostDb;
        private Dictionary<int, DbConnection> _tenantDbs; //only used for db per tenant architecture

        protected TAOKETestBase()
        {
        ...
        protected void UsingDbContext(Action<TAOKEDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<TAOKEDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<TAOKEDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<TAOKEDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<TAOKEDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<TAOKEDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<TAOKEDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<TAOKEDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<TAOKEDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<TAOKEDbContext, Task<T>> func)
        {
            T result;
